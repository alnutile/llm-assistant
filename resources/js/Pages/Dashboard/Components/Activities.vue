<template>
    <h2 class="text-sm font-semibold leading-6 text-gray-900 dark:text-gray-200">Activity</h2>
        ...
        <ul role="list" class="mt-6 space-y-6">
            <li v-for="(activityItem, activityItemIdx) in activity" :key="activityItem.id" class="relative flex gap-x-4">
                ...
                <template v-if="activityItem.type === 'commented'">
                    <img :src="activityItem.person.imageUrl" alt="" class="relative mt-3 h-6 w-6 flex-none rounded-full bg-gray-50 dark:bg-gray-700" />
                    <div class="flex-auto rounded-md p-3 ring-1 ring-inset ring-gray-200">
                        <div class="flex justify-between gap-x-4">
                            <div class="py-0.5 text-xs leading-5 text-gray-500">
                                <span class="font-medium text-gray-900 dark:text-gray-200">{{ activityItem.person.name }}</span> commented
                            </div>
                            <time :datetime="activityItem.dateTime" class="flex-none py-0.5 text-xs leading-5 text-gray-500 dark:text-gray-200">{{ activityItem.date }}</time>
                        </div>
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-200">{{ activityItem.comment }}</p>
                    </div>
                </template>
                <template v-else>
                    ...
                    <p class="flex-auto py-0.5 text-xs leading-5 text-gray-500 dark:text-gray-200">
